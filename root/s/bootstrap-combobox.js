...
(function( $ ) {
  ...
  * ================================ */
  ...
  , lookup: function (event) {
	...
	if (this.options.lookup && this.query)
	{
		var that=this;
		this.options.lookup(this.query,function(re)
		{
			that.map={};
			for (var i=0;i<re.length;i++) that.map[re[i].t]=re[i].v;
			that.process(re.map(o=>o.t));
			that.options.items=re.length;
		});
		return;
	};
